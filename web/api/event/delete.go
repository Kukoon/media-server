...
// @Router /api/v1/event/{event_id} [delete]
// @Param event_id path string false "uuid of event"
// @Security ApiKeyAuth
func apiDelete(r *gin.Engine, ws *web.Service) {
	r.DELETE("/api/v1/event/:uuid", auth.MiddlewarePermissionParamUUID(ws, models.Event{}), func(c *gin.Context) {
		id := uuid.MustParse(c.Params.ByName("uuid"))
		result := ws.DB.Delete(&models.Event{ID: id})
